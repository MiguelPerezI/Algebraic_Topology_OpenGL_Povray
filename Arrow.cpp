...
void SpaceCurve::initCortinaBase(int n, double width) {
	...
	for (int i = 0; i < n; i++) {
		...
		this->aux[0].updateVector3D(0.0, sin(u1)+1.5, u1);
		this->aux[1].updateVector3D(0.0, sin(u2)+1.5, u2);

		this->curve.inputArista(0, i, this->aux[0], this->aux[1]);
	}
}

void SpaceCurve::initCortinaHead(int n, double width) {
	...
	for (int i = 0; i < n; i++) {
		...
		this->aux[0].updateVector3D(1.5, sin(u1)+1.5, u1);
		this->aux[1].updateVector3D(1.5, sin(u2)+1.5, u2);

		this->curve.inputArista(0, i, this->aux[0], this->aux[1]);
	}
}

void SpaceCurve::initSpaceCurveParabola(int n, double v0, double g, double H, double alpha, int eje, double width) {
	...
	//double g = 9.81;
	//double H = 0.0;
	//double alpha = 0.35 * 3.1415;

	double time = ((2 * v0)/g) * sin(alpha);
	printf("\n\n-->time := %lg\n\n", time);
...
//		double u2 = ((double)i * step) + step;
//
//		this->aux[0].updateVector3D(v0 * cos(alpha) * u1, H + (v0 * sin(alpha)*u1) - (0.5 * g * u1 * u1), 0);
//		this->aux[1].updateVector3D(v0 * cos(alpha) * u2, H + (v0 * sin(alpha)*u2) - (0.5 * g * u2 * u2), 0);
//
//
//		this->curve.inputArista(0, i + n/2, this->aux[0], this->aux[1]);
//	}
	...
	for (int i = 0; i < n; i++) {
		...
		if (eje == 1) {
			this->aux[0].updateVector3D(0.0, H + (v0 * sin(alpha)*u1) - (0.5 * g * u1 * u1), -1 * v0 * cos(alpha) * u1);
			this->aux[1].updateVector3D(0.0, H + (v0 * sin(alpha)*u2) - (0.5 * g * u2 * u2), -1 * v0 * cos(alpha) * u2);
		}
